  ...
  function xyChart(options_override) {
    ...
    chart.draw_legend = function (data) {
      ...
      update_sel.enter().append('g').each(function (d, i) {
        ...
        g.append("rect").attr("x", legend_offset.x).attr("y", i * 25 + 10).attr("width", 14).attr("height", 14).style("fill", get_series_color(null, i)).style("stroke", get_series_color(null, i)).style("cursor", "pointer").on("mouseover", function () {
        ...
        g.append("text").attr("x", 18 + legend_offset.x).attr("y", i * 25 + 25).attr("height", 30).attr("width", 100).style("text-anchor", "start").style("cursor", "move").style("fill", get_series_color(null, i)).on("mouseover", function () {
          chart.svg.selectAll('path.line').classed('highlight', function (d, ii) {
            return ii == i;
          }).classed('unhighlight', function (d, ii) {
            return ii != i;
